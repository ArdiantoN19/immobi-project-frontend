...
const EditKaryawan: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      const karyawan = await api.getKaryawanById(Number(id));
      if (karyawan.status === "success") {
        setFormData(karyawan.data);
        setGender(karyawan.data["gender"]);
        setIdJabatan(karyawan.data["id_jabatan"]);
      } else {
        navigate("/karyawan");
      }
    })();
  ...
  const onSubmitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await api.updateKaryawan(Number(id), payload);
  ...
  return (
    <form className="max-w-lg pb-20" onSubmit={onSubmitHandler}>
      <h1 className="mb-5 text-3xl font-bold text-slate-900">Edit Karyawan</h1>
